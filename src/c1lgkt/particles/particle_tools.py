...
def compute_midplane_punctures(t, ysol, geom: XgcGeomHandler):
    ...
    for k in range(nump):
        ...
        # Compute the z jumps when we're on the outboard side
        z_jumps = np.diff(np.sign(z - geom.zaxis)) * (r[:-1] > geom.raxis) * ((z - geom.zaxis) != 0)
        ind_ppuncs = np.argwhere(z_jumps > 0)
        ind_npuncs = np.argwhere(z_jumps < 0)
        ...
        for j in range(ind_ppuncs.shape[0]):
            tind = ind_ppuncs[j,0]
            zfrac = -z[tind] / (z[tind+1] - z[tind])

            t_ppuncs[j] = (1-zfrac)*t[tind] + zfrac*t[tind+1]
            y_ppuncs[:,j] = (1-zfrac)*ysol[k:k+5*nump:nump,tind] + zfrac*ysol[k:k+5*nump:nump,tind+1]

        for j in range(ind_npuncs.shape[0]):
            tind = ind_npuncs[j,0]
            zfrac = -z[tind] / (z[tind+1] - z[tind])

            t_npuncs[j] = (1-zfrac)*t[tind] + zfrac*t[tind+1]
            y_npuncs[:,j] = (1-zfrac)*ysol[k:k+5*nump:nump,tind] + zfrac*ysol[k:k+5*nump:nump,tind+1]
